...
//	private static final int VALUE_TEXT_COLOR = 0xFFFF8B78;
    ...
    /**
     * Label & background 中间的几何体 </br> 选中区域的背景
     */
    ...
    private void initResourcesIfNecessary() {
        if (itemsPaint == null) {
            ...
            itemsPaint.density = density;
            itemsPaint.setTextSize(TEXT_SIZE * density);
        }
        ...
        if (valuePaint == null) {
            ...
            valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG
                    | Paint.DITHER_FLAG);
            float density = getResources().getDisplayMetrics().density;
            valuePaint.density = density;
            valuePaint.setTextSize((TEXT_SIZE + 1) * density);
            // valuePaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
        }

        if (centerDrawable == null) {
            centerDrawable = getContext().getResources().getDrawable(
                    R.drawable.wheel_city_val);
        }
        ...
        setBackgroundResource(R.drawable.wheel_city_bg);
    }
    ...
    /**
     * 触摸事件的 回调方法</br> 看到了吧 如果适配器 adapter是null 的话，这里是什么都不会做的。</br>
     * 如果适配器不为空，将MotionEvent传递给手势识别器。 并判断是否已ACTION_UP </br> 如果是说明操作已结束
     * 调用justify()方法。</br> return true. 不会泄露
     */
    @Override
